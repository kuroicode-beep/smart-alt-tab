...
- 현재 버전: **v0.5.0** (M3 + 관리자 권한 앱 전환 지원 + 단일 exe 배포)
- 로컬: `C:\Projects\smart-alt-tab`
- 상태: M1·M2·M3 완료(훅 소비·선택 이동·전환·오버레이, 필터 정교화·다중 모니터·안전장치,
  설정 화면·트레이·DPI 정밀화) + exe 배포 완료. **M4(썸네일 미리보기)는 제외 확정** —
  목록 크기의 썸네일은 저시력 사용자가 식별하지 못하는데 큰 글씨 제목 자리를 뺏는다.
  사유와 재검토 조건은 `docs/prd/`의 마일스톤 절 참고. 남은 백로그 없음.
