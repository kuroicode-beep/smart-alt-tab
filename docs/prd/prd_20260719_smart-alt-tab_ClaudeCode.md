...
| M1 | Alt+Tab 가로채기 + 자체 텍스트 전환기(큰 글씨) + 선택 이동 + 전환 | 완료 (v0.2.0) |
| M2 | 창 목록 필터 정교화(cloaked/owner), 폴백·ESC 안전장치 | 완료 (v0.3.0) |
| M3 | SVIL 디자인 적용, DPI, 설정(글꼴·크기·언어) | 완료 (v0.4.0) |
| M4 | 썸네일 미리보기(선택) | **제외** (아래 사유) |
| — | 콘솔창 없는 단일 exe 배포 + 다운로드 페이지 연결 | 완료 (v0.5.0) |

### M4(썸네일 미리보기) 제외 결정 — 2026-07-24

**넣지 않기로 확정했다. 저시력 사용자에게는 없는 편이 낫다.**

- 목록에 들어갈 크기의 썸네일은 저시력 사용자가 어차피 식별하지 못한다. 판별에 기여하지
  않는 요소가 화면을 차지하는 셈이다.
- 그 자리는 이 앱의 유일한 존재 이유인 **큰 글씨 창 제목**이 써야 한다. 썸네일을 넣으면
  제목 글자 크기나 한 화면에 보이는 창 개수 중 하나를 반드시 희생하게 된다.
- 축소된 창 미리보기는 대비가 제각각이라 고대비 다크 원칙과도 충돌한다.

되살릴 조건: 썸네일을 **제목 크기를 줄이지 않고** 배치할 수 있고, 저시력 사용자에게 실제로
판별에 도움이 된다는 근거가 생겼을 때만. 그 전에는 재논의하지 않는다.
